...
export default function DetectorPage() {
  ...
  const scoreColor = result
    ? result.level === 'critical' ? '#DC2626' : result.level === 'high' ? '#FF8A5A' : result.level === 'monitor' ? '#FFC857' : '#00E5A8'
    : '#00E5A8';
  ...
  return (
    <div className="flex flex-col gap-8 max-w-3xl mx-auto">
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="max-w-[560px] mx-auto w-full p-6 rounded-2xl bg-card border border-border flex flex-col gap-5">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Company A</label>
            <input value={companyA} onChange={e => setCompanyA(e.target.value)} placeholder="e.g. Parle-G (unlisted brand)" className="bg-background border border-input rounded-xl px-4 py-2.5 text-sm text-foreground placeholder:text-muted-foreground/60 focus:outline-none focus:border-ring transition" />
          </div>
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Company B</label>
            <input value={companyB} onChange={e => setCompanyB(e.target.value)} placeholder="e.g. PARLEIND (BSE ticker)" className="bg-background border border-input rounded-xl px-4 py-2.5 text-sm text-foreground placeholder:text-muted-foreground/60 focus:outline-none focus:border-ring transition" />
          </div>
        </div>
        <button onClick={() => runDetection()} disabled={!companyA || !companyB || loading} className="w-[240px] mx-auto flex items-center justify-center gap-2 py-3 rounded-xl bg-primary text-primary-foreground text-sm font-bold disabled:opacity-40 hover:bg-primary/90 transition">
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="flex flex-wrap gap-2">
            {PRESETS.map((p, i) => (
              <button key={i} onClick={() => applyPreset(p)} className="text-[10px] font-bold px-3 py-1.5 rounded-lg border border-border text-muted-foreground hover:text-foreground hover:border-foreground/30 transition">
      ...
      {result && (
        <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.4, ease: [0.16, 1, 0.3, 1] }} className="p-6 rounded-2xl bg-card border flex flex-col gap-6" style={{ borderColor: `${scoreColor}30` }}>
            ...
            {[
              ...
              <div key={label} className="flex flex-col gap-2 p-3 rounded-xl bg-muted border border-border">
                ...
                <div className="h-1.5 rounded-full bg-border overflow-hidden">
                  <motion.div initial={{ width: 0 }} animate={{ width: `${value}%` }} transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }} className="h-full rounded-full" style={{ backgroundColor: scoreColor }} />
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="flex gap-3">
            ...
            <button className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl bg-primary text-primary-foreground text-xs font-bold hover:bg-primary/90 transition">Save Investigation <ArrowRight className="w-3.5 h-3.5" /></button>
          </div>
        </motion.div>
      )}
    </div>
