...
const sevStyles: Record<string, { text: string; bg: string; border: string; dot: string }> = {
  critical: { text: 'text-destructive', bg: 'bg-destructive/8', border: 'border-destructive/20', dot: 'bg-destructive' },
  high:     { text: 'text-orange-400', bg: 'bg-orange-400/8', border: 'border-orange-400/20', dot: 'bg-orange-400' },
  medium:   { text: 'text-warning', bg: 'bg-warning/8', border: 'border-warning/20', dot: 'bg-warning' },
  low:      { text: 'text-success', bg: 'bg-success/8', border: 'border-success/20', dot: 'bg-success' },
};
    ...
    <div className="flex flex-col gap-6 max-w-4xl">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">All triggered confusion, hype, and anomaly alerts sorted by severity.</p>
        </div>
        <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-muted/80 border border-border text-xs font-bold text-foreground">
      ...
      <div className="flex flex-wrap gap-2">
        {(['all', 'critical', 'high', 'medium', 'low'] as SevFilter[]).map(f => (
          <button key={f} onClick={() => setFilter(f)}
            className={`px-3 py-1.5 rounded-xl text-xs font-bold capitalize transition ${filter === f ? 'bg-primary/10 text-primary border border-border' : 'bg-card text-muted-foreground border border-border hover:text-foreground hover:border-muted-foreground/30'}`}>
            {f === 'all' ? 'All Alerts' : f}
          </button>
        ))}
      </div>
        ...
        {filtered.map((alert, i) => {
          ...
          return (
            <motion.div key={alert.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.07 }}
              className={`relative flex gap-4 p-5 rounded-2xl border transition ${isResolved_ ? 'bg-muted/30 border-border opacity-50' : `glass-panel ${s.bg} ${s.border}`}`}>
              ...
              <div className="flex-1 min-w-0 flex flex-col gap-2">
                ...
                <div className="flex items-center justify-between pt-1">
                  <span className="flex items-center gap-1.5 text-[10px] text-muted-foreground font-medium"><Clock className="w-3 h-3" />{alert.createdAt}</span>
                  {!isResolved_ ? (
                    ...
                    <span className="flex items-center gap-1 text-[10px] font-bold text-muted-foreground"><CheckCircle className="w-3.5 h-3.5" /> Resolved</span>
                  )}
                </div>
              </div>
            </motion.div>
